...
class ResultMenu:
    def __init__(self, width, height, ok_btn_func, stats):
        ...
        self.gained_items_label = Label(0, 0, f"Всего предметов получено: {self.stats['enemies_killed']}", 30)
        self.inventory_label = Label(0, 0, f"Уникальных предметов: {self.stats['inventory_size']}", 30)
        self.inventory_items_label = Label(0, 0, f"Всего предметов в инвентаре: {self.stats['items_left']}", 30)
        self.buttons_layout.add_widget(self.levels_label)
        self.buttons_layout.add_widget(self.enemies_label)
        self.buttons_layout.add_widget(self.gained_items_label)
        self.buttons_layout.add_widget(self.inventory_label)
        self.buttons_layout.add_widget(self.inventory_items_label)
